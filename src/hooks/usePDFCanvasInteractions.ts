...
export const usePDFCanvasInteractions = ({
  ...
  const detectWallWithOpenCV = async (
    ...
    try {
      ...
      // If in debug mode, show what we're processing
      if (isDebugMode) {
        const dbgCanvas = ensureDebugCanvas();
        const dbgCtx = dbgCanvas.getContext('2d');
        if (dbgCtx) {
          dbgCanvas.width = roiWidth;
          dbgCanvas.height = roiHeight;
      ...
      // If in debug mode, show edge detection results
      if (isDebugMode) {
        const dbgCanvas = ensureDebugCanvas();
        const dbgCtx = dbgCanvas.getContext('2d');
        if (dbgCtx) {
          // Put the edges image on the debug canvas
          window.cv.imshow(dbgCanvas, edges);
        }
      }
      
      // Find lines using Hough Line Transform
      const lines = new window.cv.Mat();
      window.cv.HoughLinesP(edges, lines, 1, Math.PI / 180, 50, 30, 10);
      
      console.log(`HoughLinesP found ${lines.rows} lines`);
      ...
      // Draw lines on debug canvas
      if (isDebugMode && detectedLines.length > 0) {
        const dbgCanvas = ensureDebugCanvas();
        const dbgCtx = dbgCanvas.getContext('2d');
        if (dbgCtx) {
          dbgCtx.strokeStyle = 'blue';
          dbgCtx.lineWidth = 2;
          
          detectedLines.forEach(line => {
            dbgCtx.beginPath();
            dbgCtx.moveTo(line.x1, line.y1);
            dbgCtx.lineTo(line.x2, line.y2);
            dbgCtx.stroke();
            
            // Display line length and angle
            dbgCtx.fillStyle = 'red';
            dbgCtx.font = '10px Arial';
            dbgCtx.fillText(`L: ${line.length.toFixed(1)}`, line.midX, line.midY);
          });
        }
      }
      ...
      if (detectedLines.length > 0) {
        ...
        // Estimate wall thickness based on selection height
        // This is a critical value for finding similar walls
        const estimatedThickness = Math.min(10, Math.max(2, selectionRect.height / 8));
        
        console.log(`Estimated wall thickness: ${estimatedThickness}px`);
  ...
  // Improved similar wall detection - with better threshold calculation and edge tracing
  const findSimilarWalls = (): WallSymbol[] => {
    ...
    try {
      ...
      console.log(`Finding walls with similar thickness to: ${referenceThickness}px`);
      ...
      // Apply binary threshold instead of adaptive for better global detection
      const threshold = new window.cv.Mat();
      window.cv.threshold(
        blurred,
        threshold,
        200,  // Threshold value
        255,  // Max value
        window.cv.THRESH_BINARY_INV  // Binary inverse threshold
      );
      
      // Apply edge detection
      const edges = new window.cv.Mat();
      window.cv.Canny(threshold, edges, 30, 150); // Lower threshold to detect more edges
      
      // If in debug mode, show edge detection results
      if (isDebugMode) {
        const dbgCanvas = ensureDebugCanvas();
        dbgCanvas.width = pdfCanvas.width;
        dbgCanvas.height = pdfCanvas.height;
        window.cv.imshow(dbgCanvas, edges);
      }
      
      // Use more sensitive parameters for line detection
      const lines = new window.cv.Mat();
      window.cv.HoughLinesP(
        ...
        1,  // rho
        Math.PI / 180,  // theta
        30,  // threshold (lower value to detect more lines)
        20,  // minLineLength
        10   // maxLineGap
      );
      ...
      // If in debug mode and lines were found, show them
      if (isDebugMode && lines.rows > 0) {
        const dbgCanvas = ensureDebugCanvas();
        const dbgCtx = dbgCanvas.getContext('2d');
        if (dbgCtx) {
          dbgCtx.strokeStyle = 'green';
          dbgCtx.lineWidth = 1;
  ...
  const handleSelectionEnd = async (e: React.MouseEvent) => {
    ...
    if (onSimilarWallsDetected && detectedWall) {
      ...
      if (openCVLoaded && window.cv) {
        // Wait a bit for the UI to update with the first wall
        setTimeout(() => {
          ...
          if (similarWalls && similarWalls.length > 0) {
            ...
            toast({
              title: "Wall Detected",
              description: "Detected the selected wall. No similar walls were found. Try adjusting your selection or enable debug mode (Alt+D)."
            });
          }
        }, 500);
      } else {
